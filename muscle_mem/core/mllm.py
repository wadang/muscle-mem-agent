...
from muscle_mem.core.engine import (
    LMMEngineAnthropic,
    LMMEngineAnthropicLR,
...
class LMMAgent:
    def __init__(self, engine_params=None, system_prompt=None, engine=None):
        if engine is None:
            if engine_params is not None:
                ...
                elif engine_type == "anthropicLR":
                    self.engine = LMMEngineAnthropicLR(**engine_params)
    ...
    def add_message(
        ...
        elif isinstance(self.engine, (LMMEngineAnthropic, LMMEngineAnthropicLR)):
            # infer role from previous message
            if role != "user":
                if self.messages[-1]["role"] == "system":
                    role = "user"
    ...
    def get_response(
        ...
        **kwargs,
        ...
        def _normalize_messages_for_llm(
            message_list: Optional[List[Dict[str, Any]]],
        ) -> List[Dict[str, Any]]:
            if not message_list:
                return []
        ...
        messages = _normalize_messages_for_llm(messages)
        if user_message is not None:
            ...
            if text_value.strip():
                messages.append(
                    {"role": "user", "content": [{"type": "text", "text": text_value}]}
                )
    ...
    def get_response_with_thinking(
        ...
        **kwargs,
        ...
        if callable(generate_with_thinking):
